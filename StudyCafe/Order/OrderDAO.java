...
public class OrderDAO {
    ...
    public void insertOrder(int userIdx, int drinkIdx, int total_price, int seatIdx, int usingTime) throws SQLException {
        ...
        try {
            ...
            cstmt.setInt(2, drinkIdx);
            cstmt.setInt(3, total_price);
            cstmt.setInt(4, seatIdx);
            cstmt.setInt(5, usingTime);

            cstmt.execute();
            System.out.println("주문 데이터가 성공적으로 삽입되었습니다.");

        } catch (SQLException e) {
            e.printStackTrace();
            System.out.println("주문 데이터 삽입 중 오류가 발생했습니다.");
        } finally {
